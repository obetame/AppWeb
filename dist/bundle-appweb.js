...
/***/ function(module, exports, __webpack_require__) {
	...
	var _vueMaterial = __webpack_require__(5);
	...
	var Vue_App = new _vue2.default({
		...
		created: function created() {
			...
			window.getData = function (data) {
				if (data) {
					...
					_this.testOut = "无返回值(可能报错,建议加入返回值查看是否调用成功)";
					return _this.testOut;
				}
			};
		...
		methods: {
			c_testInput: function c_testInput() {
			...
			timeTask: function timeTask() {
				...
				setTimeout(function () {
					if (Object.prototype.toString.call(_this2.timeTaskValue).replace(/\[object (\w*)\]/, "$1").toLowerCase() === 'null') {
						_this2.testOut = "无返回值(可能报错,建议加入返回值查看是否调用成功)";
					} else {
						_this2.testOut = _this2.timeTaskValue;
						_this2.timeTaskValue = null;
					}
				}, 100);
			},
			c_clearTestInput: function c_clearTestInput() {
				this.testInput = "";
				this.testOut = "";
			},
			...
			getLocalStore: function getLocalStore() {
				if (window.localStorage.getItem("Test") !== null) {
					//之前有数据
					var oldValue = JSON.parse(window.localStorage.getItem("Test")); //获取数据
					return oldValue;
				} else {
					// 第一次储存
					return [];
			...
			localStore: function localStore(string) {
				...
				window.localStorage.setItem("Test", (0, _stringify2.default)(newData));
			},
			goToTest: function goToTest(code) {
				this.testInput = code;
				this.c_testInput();
			},
			c_clearTestHis: function c_clearTestHis() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(false) {
		// When the styles change, update the <style> tags
		if(!content.locals) {
			module.hot.accept("!!./../node_modules/css-loader/index.js!./appweb.css", function() {
				var newContent = require("!!./../node_modules/css-loader/index.js!./appweb.css");
				if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
				update(newContent);
			});
		}
